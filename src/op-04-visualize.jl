...
# Arguments
- `op::DataFrame` -- a data frame, containing simulated operational characteristics; 
each column represents simulated output for a particular randomization procedure.
- `xlabel::String` -- a text label for ``X`` axis.
- `ylabel::String` -- a text label for ``Y`` axis.
- `colors::Matrix{Symbol}` -- a matrix (row) of colors (symbolic representation) to distinguish outputs for different 
randomization procedure.
- `shapes::Matrix{Symbol}` -- a matrix (row) of markers (symbolic representation) to distinguish outputs for different 
randomization procedure.
